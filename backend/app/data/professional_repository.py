...
class ProfessionalRepository:
    ...
    async def get_professional_by_id(self, professional_id: UUID) -> Optional[User]:
        """Return a single professional by UUID or None if not found."""
        stmt = select(User).where(User.id == professional_id)
        result = await self.session.execute(stmt)
        return result.scalar_one_or_none()

    async def get_professional_by_email(self, email: str) -> Optional[User]:
        """Return a professional by email (case-insensitive) or None."""
        stmt = select(User).where(User.email.ilike(email.lower()))
        result = await self.session.execute(stmt)
        return result.scalar_one_or_none()
    ...
    async def list_professionals(
        ...
        stmt = select(User).order_by(User.name.asc()).offset(offset).limit(page_size)
