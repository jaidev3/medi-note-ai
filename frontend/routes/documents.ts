...
export const documentsApi = {
  async uploadDocument(
    ...
    formData.append(
      "extract_text",
      uploadData.extract_text?.toString() || "false"
    );
    formData.append(
      "generate_soap",
      uploadData.generate_soap?.toString() || "false"
    );
  ...
  async processDocument(
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new ApiError(
        error.detail || "Failed to process document",
        response.status
      );
    }
  ...
  async getDocumentMetadata(
    ...
    const response = await fetch(
      `${API_BASE_URL}${API_ENDPOINTS.DOCUMENTS.GET(documentId)}`,
      {
        method: "GET",
        headers: getAuthHeaders(token),
      }
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new ApiError(
        error.detail || "Failed to get document metadata",
        response.status
      );
    }
  ...
  async deleteDocument(
    ...
    const url = new URL(
      `${API_BASE_URL}${API_ENDPOINTS.DOCUMENTS.DELETE(documentId)}`
    );
    url.searchParams.set("delete_file", deleteFile.toString());
    if (reason) {
  ...
  async getDocumentContent(
    ...
    const response = await fetch(
      `${API_BASE_URL}${API_ENDPOINTS.DOCUMENTS.CONTENT(documentId)}`,
      {
        method: "GET",
        headers: getAuthHeaders(token),
      }
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new ApiError(
        error.detail || "Failed to get document content",
        response.status
      );
    }
  ...
  async listSessionDocuments(
    ...
    const url = new URL(
      `${API_BASE_URL}${API_ENDPOINTS.DOCUMENTS.SESSION_DOCUMENTS(sessionId)}`
    );
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new ApiError(
        error.detail || "Failed to list session documents",
        response.status
      );
    }
  ...
  async listDocuments(
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new ApiError(
        error.detail || "Failed to list documents",
        response.status
      );
    }
