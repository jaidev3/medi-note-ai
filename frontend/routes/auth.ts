...
export interface RegisterRequest {
  ...
  email: string
  password: string
  role: 'Audiologists' | 'Hearing Aid Specialists' | 'ENT Physicians' | 'Clinical Support Staff'
  department?: string
  employee_id?: string
  phone_number?: string
}
...
export const authApi = {
  ...
  async refreshToken(refreshToken: string): Promise<TokenResponse> {
    ...
    if (!response.ok) {
      const error = await response.json()
      throw new ApiError(error.detail || 'Token refresh failed', response.status)
    }
  ...
  async getCurrentUser(token: string): Promise<UserProfile> {
    ...
    if (!response.ok) {
      const error = await response.json()
      throw new ApiError(error.detail || 'Failed to get user info', response.status)
    }
